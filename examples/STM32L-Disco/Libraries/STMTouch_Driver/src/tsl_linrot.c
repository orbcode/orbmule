    ...
    if (THIS_DIRECTION == TSL_TRUE) // Anticlockwise direction ...
        {
            ...
            if (((TSL_tPosition_T)new_position > THIS_RAW_POSITION) &&
                    (((TSL_tPosition_T)new_position - THIS_RAW_POSITION) < DIRECTION_CHANGE_MAX_DISPLACEMENT))
                {
    ...
    else // Clockwise direction... DEFAULT SETTING !
        {
            ...
            if (((TSL_tPosition_T)new_position < THIS_RAW_POSITION) &&
                    ((THIS_RAW_POSITION - (TSL_tPosition_T)new_position) < DIRECTION_CHANGE_MAX_DISPLACEMENT))
                {
...
void TSL_linrot_DebCalibrationStateProcess(void)
{
    if (TSL_linrot_ProcessCh_One_AcqStatusError() == TSL_STATUS_OK) // Acquisition error (min or max)
        ...
        {
            if (TSL_linrot_ProcessCh_One_DeltaBelowEquMinus(THIS_CALIB_TH,
                    1) == TSL_STATUS_OK) // Still below recalibration threshold
                {
                    if (THIS_COUNTER_DEB > 0)
                        {
                            THIS_COUNTER_DEB--;
