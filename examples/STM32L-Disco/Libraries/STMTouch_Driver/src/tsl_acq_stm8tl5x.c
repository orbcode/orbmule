                    ...
                    if (bank->msk_channels & (uint16_t)((uint16_t)1 << idx_ch))
                        {
                            ...
                            if (!(PXS->RXSR & (uint16_t)((uint16_t)1 << idx_ch)) ||
                                    (PXS->RXCNTR[idx_ch] > (TSLPRM_KEY_TARGET_REFERENCE + TSLPRM_KEY_TARGET_REFERENCE_ERROR)))
                                {
                                    if (currentEPCC > 0)
                                        {
...
#endif
            if ((reference != 0) && ((reference > (TSLPRM_KEY_TARGET_REFERENCE + target_error)) ||
                                     (reference < (TSLPRM_KEY_TARGET_REFERENCE - target_error))))
                {
